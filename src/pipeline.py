...
def run_query(query: str, search: HybridSearch, reranker: CrossEncoderReranker) -> tuple[str, list[str]]:
    ...
    if not OPENAI_API_KEY:
        raise RuntimeError("OPENAI_API_KEY is required for real LLM generation.")
    ...
    resp = client.responses.create(
        model=OPENAI_MODEL,
        instructions="Trả lời CHỈ dựa trên context. Nếu không có thông tin phù hợp, nói 'Không tìm thấy.'",
        input=f"Context:\n{context_str}\n\nCâu hỏi: {query}",
